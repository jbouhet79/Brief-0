...
console.log(Number(nombre1) + Number(nombre2)); // erreur : il faut convertir les chaînes de caractères en nombres
//--> dans console.log remplacer nombre1 et nombre2 par Number(nombre1) et Number(nombre2) 
//--> ou bien nombre1 = parseInt(nombre1);
...
nombrePair(); //--> appeler la fonction
...
if (nombre % chiffre == 0) { // --> === et rajouter Number pour vérifier que la saisie soit bien un number
...
window.addEventListener("contextmenu", function (e) {
  ...
  alert("Vous avez fait un clic droit"); // --> pas de modif
});
...
window.addEventListener("load", function () {
  ...
  titre.style.color = "red";  //--> Changer getElementById par querySelector
});
...
for (var i = 0; i < petitTableau1.length; i++) {
  console.log(petitTableau1[i]); // --> pas de chgt : ok
}
